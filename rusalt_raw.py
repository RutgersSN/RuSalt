...
def extract(fs=None):
    ...
    for f in fs:
        ...
        iraf.apall(input=f + '[SCI]', output=outbase, interactive='yes',                           
                   recenter='no', review='no', line='INDEF', nsum=-1000,
                   lower=-5, upper=5,b_function='legendre', b_order=5,
...
def trim(fs=None):
	...
	for i,f in enumerate(fs):
		...
		hdu= pyfits.open(f)
		npix=hdu[0].data.shape[2]
		W1=int(w.wcs_pix2world(40,0,0,0)[0])
		W2=int(w.wcs_pix2world(npix-40,0,0,0)[0])
